...
import com.mch.common.exception.CaptchaIncorrectException;
import com.mch.common.shiro.MyProjectProperties;
import com.mch.common.util.CaptchaUtil;
import com.mch.common.util.ResultBean;
import com.mch.system.model.User;
import com.mch.system.service.MailService;
...
public class LoginController {
    ...
    public String login(Model model) {
        model.addAttribute("loginVerify", myProjectProperties.getLoginVerify());
        return "login";
    }
    ...
    public ResultBean login(User user, @RequestParam(value = "captcha", required = false) String captcha) {
        ...
        // 如果开启了登录校验
        if (myProjectProperties.getLoginVerify()) {
    ...
    public ResultBean register(User user) {
        ...
        new Thread(() ->
                mailService.sendHTMLMail(user.getEmail(), "my-project 激活邮件", mailContent))
                .start();
